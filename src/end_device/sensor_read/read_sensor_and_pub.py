# Name: read_sensor.py
# Original Code Provided by: Dr. Jeff Prevost
# Code modified exclusively by: Jonathan Lwowski
...
if __name__ == '__main__':
	...
	cloud_connection = pika.BlockingConnection(pika.ConnectionParameters(host=cloud_hostname, credentials=cloud_credentials, port=cloud_port))
	cloud_channel = cloud_connection.channel()
	cloud_channel.exchange_declare(exchange='temp_from_pi', exchange_type='direct')
	edge_connection = pika.BlockingConnection(pika.ConnectionParameters(host=edge_hostname, credentials=edge_credentials, port=edge_port))
